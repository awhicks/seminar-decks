...
- `index.html` — **the deck: put your real lecture slides here** (each slide
  is a `<section>`, speaker notes go in `<aside class="notes">`). Also
  configures the seminar room.
...
- **Projector / audience Mac**: `http://localhost:8000/csc201/`
  (auto-joins the room as a participant)
- **iPad**: `http://<mac-ip>:8000/presenter.html?deck=/csc201/index.html`
  → enter a presenter name + the host password → you are hosting.
...
- The room has an explicit venue (`url: 'CSC201'`), so it does not matter
  whether someone opens the deck via `localhost` or via the LAN IP —
...
```text
http://<mac-ip>:8000/presenter.html?deck=/csc201/index.html
```
...
Copy this `csc201/` folder (e.g. to `csc301/`) inside the reveal.js
repo, then edit the copy's `index.html`:

- Replace the example `<section>`s with your slides (speaker notes in
  `<aside class="notes">`).
- Change `seminar.room` (e.g. `'CSC301'`) so each course gets its own
  room.
